...
describe(`Icon`, () => {
  // Every entry, not a sample: the set is merged from another repo, and a `path` holding
  ...
  test(`every icon renders its viewBox, fill, stroke and shape`, () => {
    ...
    for (const [name, entry] of Object.entries(icon_data)) {
      ...
      if (path.trimStart().startsWith(`<`)) {
        // the regression this guards: markup rendered into a `d` attribute
        if (svg.innerHTML.includes(`d="<`)) offenders.push(`${name}: markup in d`)
      } else if (svg.querySelector(`path`)?.getAttribute(`d`) !== path) {
