...
const default_labels = {
  ready: { icon: icons.Copy, text: `ready` },
  success: { icon: icons.Check, text: `success` },
  error: { icon: icons.Alert, text: `error` },
} as const
...
test.each([
  ...
  const { copy_button } = mount_copy_button({
    labels: {
      ready: { icon: icons.Copy, text },
      success: { icon: icons.Check, text },
      error: { icon: icons.Alert, text },
    },
  })
  const wrapper = copy_button.querySelector(`span`)
  expect(wrapper?.querySelectorAll(`span`)).toHaveLength(expected_spans)
  expect(icon_path(copy_button)).toBe(icons.Copy.d)
...
test.each([
  ...
  async (expected_state, rejection, icon) => {
    ...
    expect(icon_path(copy_button)).toBe(icons[icon].d)
